...
// Frame mux shared by every chat (companion) identity: the phone app talks to
// one instance over TCP while the web panel injects app-protocol frames into
// the same mesh, and a mirror archive lets the web read history without
// consuming the device's offline queue. Each chat identity owns one of these.
