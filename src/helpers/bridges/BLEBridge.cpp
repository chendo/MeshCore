...
void BLEBridge::onLinkFrame(const uint8_t* data, uint16_t len, uint8_t link_idx) {
  ...
  /* dispatch() prefers an established link, so on a linked pair MOST bridged
     packets arrive here rather than by broadcast. Hooking only the broadcast
     path left the clock sampler seeing a small minority of the traffic. */
  if (_raw_observer) _raw_observer(&data[HEADER_SIZE], (uint8_t)(signed_len - HEADER_SIZE));
